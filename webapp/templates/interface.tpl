...
function platformSelect(version) {ldelim}
  if (!version)
    return;
  window.location.href = '{$ROOT}/platform/' + version + '/interface/{$interface|escape:'url'}';
{rdelim}

function diffSelect(version) {ldelim}
  if (!version)
    return;
  window.location.href = '{$ROOT}/compare/interface/{$interface|escape:'url'}/{$interface->platform->version|escape:'url'}/' + version;
{rdelim}
...
{if $showdiff eq "true"}
  <p class="navbox">
    ...
    <select onchange="diffSelect(this.value)">
      ...
      {foreach from=$interface->versions item="item"}
        {if $item->hash ne $interface->hash}
          <option value="{$item->platform->version|escape:'url'}">{$item->platform|escape}</option>
        {/if}
      {/foreach}
    </select>
  </p>
...
<p class="navbox">
  ...
  <a href="{$ROOT}/platform/{$interface->platform->version|escape:'url'}/interface/{$interface->name|escape:'url'}/usage">Interface Usage</a>
</p>
<p class="navbox">
  <img src="https://developer.mozilla.org/favicon.ico" />
  <a href="https://developer.mozilla.org/en/{$interface->name|capitalize:true|escape:'url'}">More information</a>
</p>
...
<div class="body">
  <div class="idl">
    <pre class="comment">{$interface->comment|escape}</pre>
